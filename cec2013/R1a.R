...
x2 <- seq(-20, 50, by = 0.1)          # ... like -500, -495, -490, ... , 495, 500; for x1 and x2 each
 # y  <- outer(x1, x2, Vectorize(cecF))  # calc function value ...
# ... outer() - apply function Vectorize(cecF) to two arrays (x1, x2)
# ... Vectorize(cecF) vectorizes action of function cecF - makes it able to accept x1 and x2 vectors as its arguments
# draw 3D chart
# :param: bl2gr.colors - vector with palette of colors
# :param: phi, theta   - angles defining the viewing direction, azimuthal, colatitude respectively
 # persp3D(x1, x2, y, theta = 50, phi = 20, color.palette = bl2gr.colors)  

# draw section:
# filled.contour(x1, x2, y, color.palette = bl2gr.colors)
...
for(i in 1:5) {
  GA <- ga(type = "real-valued",                       # for optimization problems where the decision variables are floating-point representations of real numbers
           fitness =  function(x) -cecF(x[1], x[2]),   # fitness function - takes as input an individual string representing a potential solution, and returns a numerical value describing its “fitness”.
